...
assert.equal(vm.runInContext('quizTotalQuestions()',context),36);
...
assert.ok(vm.runInContext('blocks.every(block=>block.sequences.every(sequence=>sequence.commandIds.every(id=>lessons.some(lesson=>lesson.id===id))))',context));
assert.equal(vm.runInContext("blocks[0].sequences[0].commandIds.join(',')",context),'man,pwd,cd,ls');
...
assert.doesNotMatch(element('view').innerHTML,/Cours à venir/);
...
assert.equal(vm.runInContext('lessons.length',context),37);
assert.ok(vm.runInContext("lessons.every(lesson=>!lesson.html.includes('Pour s’entraîner'))",context));
context.location.hash='#fiche-ls';
vm.runInContext('render()',context);
assert.match(element('view').innerHTML,/ls -la/);
assert.match(element('view').innerHTML,/entrées cachées/);
...
assert.match(element('view').innerHTML,/<span class="terminal-output">\.  \.\.  \.secret  config\.txt<\/span>/);
assert.doesNotMatch(element('view').innerHTML,/ls -lh/);
assert.match(element('view').innerHTML,/Faire le QCM/);
context.location.hash='#fiche-man';
vm.runInContext('render()',context);
assert.match(element('view').innerHTML,/man -k dossier/);
assert.match(element('view').innerHTML,/help cd/);
assert.match(element('view').innerHTML,/<span class="terminal-output">NAME<\/span>/);
context.location.hash='#fiche-pwd';
vm.runInContext('render()',context);
assert.match(element('view').innerHTML,/pwd -L/);
assert.match(element('view').innerHTML,/pwd -P/);
assert.match(element('view').innerHTML,/\$PWD/);
assert.match(element('view').innerHTML,/<span class="terminal-output">\/home\/alice\/atelier<\/span>/);
context.location.hash='#fiche-cd';
vm.runInContext('render()',context);
assert.match(element('view').innerHTML,/cd \.\.\/\.\./);
assert.match(element('view').innerHTML,/cd -/);
assert.match(element('view').innerHTML,/\$HOME/);
assert.match(element('view').innerHTML,/\$OLDPWD/);
assert.match(element('view').innerHTML,/chemin relatif/i);
assert.match(element('view').innerHTML,/<span class="terminal-output">\/home\/alice<\/span>/);
assert.doesNotMatch(element('view').innerHTML,/Pour s’entraîner/);
...
assert.match(element('view').innerHTML,/36 questions/);
...
context.location.hash='#qcm-cd';
vm.runInContext('render()',context);
assert.match(element('view').innerHTML,/QUESTION 1 \/ 4/);
assert.match(vm.runInContext("quizzes.find(quiz=>quiz.id==='cd').questions.map(question=>question.prompt+' '+question.options.map(option=>option.text).join(' ')).join(' ')",context),/\$OLDPWD/);
assert.match(vm.runInContext("quizzes.find(quiz=>quiz.id==='cd').questions.map(question=>question.prompt).join(' ')",context),/Mes documents/);
vm.runInContext("quizFeedback.cd={valid:false,selected:[0,3]}; render()",context);
assert.match(element('view').innerHTML,/quiz-option selected incorrect/);
assert.match(fs.readFileSync(path.join(root,'styles.css'),'utf8'),/\.quiz-question \.quiz-option\.incorrect\{border-color:#b9674d;background:#f9e4dd\}/);
...
console.log('Interface : Bloc 01, 3 modules CTF, 37 fiches, 16 QCM et box à venir vérifiés.');
